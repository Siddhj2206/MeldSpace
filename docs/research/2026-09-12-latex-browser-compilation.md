...
- **Headers — not required for `texlyre-busytex` (spike, 2026-09-12).** The claim that BusyTeX-class engines need `SharedArrayBuffer` (and so `Cross-Origin-Opener-Policy: same-origin` + `Cross-Origin-Embedder-Policy: require-corp`) comes from **Siglum**, a different packaging. Spike findings: the BusyTeX wasm link flags carry no `-sUSE_PTHREADS`; the shipped Emscripten glue has no `SharedArrayBuffer`, `Atomics`, or `crossOriginIsolated` references; `busytex_worker.js` is a classic single-threaded `importScripts` worker. **Conclusion: `texlyre-busytex` is single-threaded — no cross-origin isolation, no COOP/COEP.** ([Siglum](https://github.com/SiglumProject/siglum))
...
7. **Headers + PWA — no COOP/COEP needed.** The spike found `texlyre-busytex` is single-threaded (no `SharedArrayBuffer`/`Atomics` in the glue, a classic `importScripts` worker, no `-sUSE_PTHREADS` in the wasm link), so cross-origin isolation is not required and the y-webrtc/auth surfaces are untouched by headers. The real constraint is **asset weight**: ~32 MB wasm + 93 MB `texlive-basic` (up to ~400 MB with recommended/extra), which the PWA service worker must be taught to cache (it currently excludes these types).
...
- **COOP/COEP is resolved: not required.** The earlier statement here — that BusyTeX-class engines require `SharedArrayBuffer` and therefore COOP/COEP — came from Siglum, not from `texlyre-busytex`. The shipped engine is single-threaded: no `SharedArrayBuffer`, `Atomics`, or `crossOriginIsolated` in any engine JS, no `-sUSE_PTHREADS` in the wasm link flags, and a classic `importScripts` worker. Static verification only; a live in-browser compile is the remaining confirmation.
